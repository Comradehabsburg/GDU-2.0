...
NDefines.NGame.LAG_DAYS_FOR_LOWER_SPEED = 400				-- Days of client lag for decrease of gamespeed
...
NDefines.NDiplomacy.MP_NUM_WINNERS_FOR_PEACE_PAUSE = 5			-- Number of human players as winners needed for peace conference to pause and open for everyone in multiplayer needs to fullfill MP_NUM_LOSERS_FOR_PEACE_PAUSE as well.
NDefines.NDiplomacy.MP_NUM_LOSERS_FOR_PEACE_PAUSE = 5			-- Number of human players as losers needed for peace conference to pause and open for everyone in multiplayer needs to fullfill MP_NUM_WINNERS_FOR_PEACE_PAUSE as well.
...
NDefines.NCountry.AIR_SUPPLY_CONVERSION_SCALE = 0.10				-- Conversion scale for planes to air supply
...
NDefines.NMilitary.LAND_COMBAT_STR_DAMAGE_MODIFIER = 0.15        -- global damage modifier... but some equipment is returned at end of battles see : EQUIPMENT_COMBAT_LOSS_FACTOR
...
NDefines.NMilitary.EQUIPMENT_COMBAT_LOSS_FACTOR = 1.1	 	   -- % of equipment lost to strength ratio in combat so some % is returned if below 1
...
NDefines.NAir.NAVAL_STRIKE_TARGETTING_TO_AMOUNT = 1		-- Balancing value to convert the naval_strike_targetting equipment stats to chances of how many airplanes managed to do successfull strike.
NDefines.NAir.NAVAL_STRIKE_DAMAGE_TO_STR = 3					-- Balancing value to convert damage ( naval_strike_attack * hits ) to Strength reduction.
